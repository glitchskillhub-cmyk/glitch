  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">My Programs</h1>
          <p className="text-slate-600 text-base mt-2">Continue your learning journey.</p>
        </div>
      </div>

      {/* Enrolled Courses */}
      <div className="space-y-6">
        <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-3">
        ...
        {activePrograms.length > 0 ? (
            ...
            {activePrograms.map((program, i) => (
              <div key={i} className="bento-card bg-white flex flex-col md:flex-row gap-10 p-10 border-slate-200 group">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <h3 className="text-xl font-bold text-slate-900 mb-2">{program.course?.title || program.title}</h3>
                  <p className="text-slate-600 text-sm mb-6 font-medium line-clamp-2">
                  ...
                  <div className="space-y-6 mb-8">
                     <div className="flex justify-between items-end mb-2">
                        <p className="text-xs font-semibold text-primary uppercase tracking-wider">Progress: {program.progress || 0}%</p>
                        <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Target: Completion by June</p>
                     </div>
                     <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-primary" style={{ width: `${program.progress || 0}%` }}></div>
                     </div>
                     <p className="text-sm font-medium text-slate-600 italic">
                       Next up: <span className="text-slate-900 not-italic font-semibold">{program.nextLesson || 'Getting Started'}</span>
                     </p>
                  </div>
          ...
          <div className="bento-card bg-white p-16 text-center border-dashed border-slate-200">
            ...
            <h3 className="text-xl font-bold mb-4">No Active Programs</h3>
            <p className="text-slate-600 mb-10 max-w-sm mx-auto">You haven't enrolled in any program yet. Enroll now to start your high-performance career journey.</p>
            <a href="#available-programs" className="btn-premium py-4 px-8 inline-flex items-center justify-center gap-2">
      ...
      <div id="available-programs" className="space-y-6 pt-10 border-t border-slate-100">
        ...
        {coursesToShow.length > 0 ? (
              ...
              <div key={course._id} className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden group hover:border-primary/30 transition-all duration-500 flex flex-col h-full shadow-sm hover:shadow-2xl">
                 <div className="relative h-48 overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute bottom-6 left-6">
                       <span className="bg-primary text-black px-4 py-1.5 rounded-full text-xs font-bold tracking-wide shadow-lg">
                 ...
                 <div className="p-8 space-y-6 flex-1 flex flex-col justify-between">
                    <div className="space-y-3">
                       ...
                       {course.startDate && (
                         <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 flex items-center gap-1.5 pt-2">
                            <Clock size={12} className="text-primary" />
                            Starts: <span className="text-slate-900 font-bold">{course.startDate}</span>
                         </p>
                       )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Full Course</p>
                          <p className="text-lg font-bold text-slate-900">₹{course.price || '9,999'}</p>
                       </div>
                       <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Slot Booking</p>
                    ...
                    <div className="pt-4 border-t border-slate-100 flex flex-col gap-4 mt-auto">
                       <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                             <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center font-bold text-xs text-slate-500 border border-slate-200">
                                {course.instructor ? course.instructor.charAt(0) : 'G'}
                             </div>
                             <p className="text-xs font-semibold tracking-wider text-slate-500">{course.instructor || 'Glitch Team'}</p>
                          </div>
                          ...
                          {course.readMoreLink && (
                            <a 
                              ...
                              className="text-xs font-semibold tracking-wider text-primary hover:underline flex items-center gap-1 group/btn"
                            >
          ...
          <div className="bento-card bg-white p-12 text-center border-dashed border-slate-200">
             ...
             <p className="text-slate-500 text-sm font-medium tracking-wide">All programs are already in your active curriculum!</p>
          </div>
        )}
      </div>
           ...
           {upcomingPrograms.map((program, i) => (
             <div key={i} className="bento-card bg-slate-50 border-dashed border-slate-200 p-8 flex items-center gap-6 grayscale opacity-60">
                ...
                <div>
                   ...
                   <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">{program.status}</span>
                </div>
                <button className="ml-auto bg-white border border-slate-200 text-slate-500 px-4 py-2 rounded-lg text-xs font-semibold tracking-wide hover:bg-primary hover:text-slate-900 hover:border-primary transition-all">Notify Me</button>
             </div>
           ))}
        </div>
      </div>
